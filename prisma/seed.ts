...
async function main() {
  console.log('检查是否需要灌入种子数据…');

  // ============ 只在数据库为空时灌入 ============
  const existingChannels = await prisma.channel.count();
  const existingReplays = await prisma.replayVideo.count();

  if (existingChannels > 0 || existingReplays > 0) {
    console.log(`数据库已有数据：频道 ${existingChannels} 条，回放 ${existingReplays} 条`);
    console.log('跳过 seed');
    return;
  }

  console.log('数据库为空，开始灌入种子数据…');
